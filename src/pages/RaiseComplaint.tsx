...
const RaiseComplaint: React.FC = () => {
  ...
  const verifyWithAI = async (imageBase64: string, title: string, description: string, category: string) => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:5001/api/complaints/verify', {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const base64Data = image!.replace(/^data:image\/\w+;base64,/, '');
      const res = await fetch('http://localhost:5001/api/complaints/create', { method: 'POST', headers: { 'Content-Type': 'application/json', ...(token && { 'Authorization': `Bearer ${token}` }) }, body: JSON.stringify({ ...form, imageBase64: base64Data, submittedBy: user.id, submittedByName: user.name, institute: user.institute, coordinates: userLocation }) });
