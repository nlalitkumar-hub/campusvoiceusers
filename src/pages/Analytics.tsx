    ...
    const load = async () => {
      ...
      try {
        ...
        if (token && !token.startsWith('firebase_')) {
          const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/analytics/summary`, {
  ...
  useEffect(() => {
    const loadPersonal = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        if (token && !token.startsWith('firebase_')) {
          const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/analytics/personal`, {
            headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }
          });
          if (res.ok) {
            const data = await res.json();
